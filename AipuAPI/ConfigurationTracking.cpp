...
void ConfigurationTracking::SetValueJSONToConfiguration() {
	...
	for (Json::Value::const_iterator it = params.begin();
		...
		if (it.key().asString() == FACE_DISCOVERY_FREQUENCE_MS) {
			discoveryFrequenceMS = it->asInt();
		}
		else if (it.key().asString() == COUNT_REDETECT_TIME_DELTA) {
			countRedetectTimeDelta = it->asInt();
		}
